...
concerns or the bottleneck of clinical annotation labour~\cite{du2023boostingdermatoscopiclesionsegmentation}. This motivation 
is grounded in our own experience when developing a dermoscopic lesion segmentation 
model, we found that existing public datasets provided insufficient annotated samples 
to improve generalization, while recruiting trained dermatologists for pixel-level 
mask annotation is rarely feasible at scale.
...
but typically lack fine-grained spatial control~\cite{SheJun_DiDGen_MICCAI2025,xu2025skindualgenpromptdrivendiffusionsimultaneous}. Combining latent 
...
\begin{itemize}[leftmargin=1.5em]
    \item \textbf{Harmonized multi-source corpus.} We unify ISIC-2018 and HAM10000 into a standardized 11k-image collection with consistent resolution, label taxonomy, and binary segmentation masks.
    \item \textbf{Physics-based optics simulation.} We design and apply a circular aperture, vignetting, and barrel-distortion pipeline that aligns heterogeneous source images to a canonical dermoscope appearance, reducing domain shift at no annotation cost.
    ...
    \item \textbf{Clinical utility validation.} We demonstrate through a rigorous TSTR protocol that a segmentation model trained \emph{only} on synthetic data surpasses an ImageNet baseline by $+13.4\%$ IoU on real patient images, confirming the translational value of the generated data.
\end{itemize}
