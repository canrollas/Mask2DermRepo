...
===========================================
Evaluates mask fidelity at cs ∈ {0.5, 0.8, 1.0}.

Usage:
    python evaluate/exp6_cn_scale.py \
        --controlnet  outputs/mask2derm/controlnet-final \
        --seg_model   outputs/eval_seg/unet.pth \
        --data_dir    data/processed \
        --output_dir  results/exp6_cn_scale \
        --device      cuda
"""
...
def main():
    ...
    p.add_argument("--output_dir",  default="results/exp6_cn_scale")
    ...
    for cs in CN_SCALE_VALUES:
        tag = f"cn_{cs:.1f}".replace(".", "_")
        print(f"\n=== ControlNet scale cs={cs} ===")
        ...
        per_scale[str(cs)] = {"fid": fid, **m}
        print(f"  FID={fid:.3f}  Dice={m['dice_mean']:.4f}  IoU={m['iou_mean']:.4f}")

    results = {"per_cn_scale": per_scale, "config": vars(args)}
    print_results({f"cs={k}  FID": v["fid"] for k, v in per_scale.items()},
                  title="Ablation 6 — ControlNet Conditioning Scale")
